...
/ WebSocket service for wimall delivery agents. It is the only source of
truth for this service's external interface; if you're integrating a client
(the agent app, an agency/customer dashboard, or wimall itself), read here,
not the source.

geo-tracker never owns user/order/shipment data. wimall ("Project A")
...
- [webhooks.md](./webhooks.md) — inbound lifecycle events from wimall (HMAC-authenticated)
...
- [tracking-notifications.md](./tracking-notifications.md) — outbound tracking-state notifications to wimall (geo-tracker → Project A)
...
This is computed by wimall (`GET /api/tracking/visible-agents`), which
...
recorded — wimall pushes an event to geo-tracker, which **immediately**
...
|---|---|---|
| wimall access token (HS256 JWT) | WebSocket + HTTP routes | `Sec-WebSocket-Protocol: bearer, <token>` on WS; `Authorization: Bearer <token>` on HTTP |
| HMAC-SHA256 signature | inbound webhooks (`/webhooks/node`, `/webhooks/agent-actions`) | `X-Node-Signature: <hex>` over the raw body |

The same token you use against wimall works here — geo-tracker verifies it
