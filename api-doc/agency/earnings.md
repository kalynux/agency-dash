...
**Error Responses**:
- `401` – `AUTH_MISSING_TOKEN` / `AUTH_TOKEN_EXPIRED` / `AUTH_TOKEN_INVALID` – Missing, expired or malformed token. ⚠ **There is no `UNAUTHORIZED` code in the registry.**
- `403` – `AUTH_ROLE_NOT_FOUND` – Valid token but not an agency. ⚠ **There is no `FORBIDDEN` code in the registry.**
