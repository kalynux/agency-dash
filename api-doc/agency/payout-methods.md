...
The schema is shared byte-for-byte with vendors and agents, so anything you learn here transfers.
...
|---|---|---|
| First time (onboarding) | `PUT /api/agency/onboarding/payout` | Step 2. Mandatory — onboarding cannot complete without it |
| Every edit afterwards | `PATCH /api/agency/profile` | The onboarding endpoint returns `409 DELIVERY_ONBOARDING_ALREADY_COMPLETED` once complete |
| Read it back | `GET /api/agency/profile` → `payoutDetails[]` | **Masked** — see [Reading it back](#reading-it-back) |

**Auth**: all three require an `agency` JWT.

> **`onboardingStep` moves with the data.** The server recalculates completeness after every save;
> payout details present is what marks Step 2 done.
...
The agency read shape is the **whole list**, in order, each entry carrying a read-only
`is_preferred` (`true` only for index 0 — never send it):
...
> **A read-back is not a round-trip.** You cannot GET the masked list, change one entry and PATCH it
...
TypeScript interfaces for the whole profile, payout included, are in
[Profile schema](./profile-schema.md#3-payout-details).
...
[Earnings — Requesting a payout](./earnings.md).

With **no** payout method configured, `POST /api/agency/earnings/payout` is refused with
