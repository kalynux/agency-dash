...
- **Two backend services, one identity.** `jovi-mall` (Express + TypeScript +
...
- **Product name: Wi Mall** (canonical, per the project owner). The strings
  "Jovi Mall" / `jovi-mall` that appear in the package name, the `VITE_APP_NAME`
  default, and the placeholder login screen are stale — treat Wi Mall as the real
  name and do not entrench "Jovi Mall" in new work.
